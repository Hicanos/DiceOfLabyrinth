...
public class SelectAdventureUIController : MonoBehaviour
{
    ...
    public void OpenSelectDungeonPanel(int chapterIndex)
    {
        ...
        else if (StageManager.Instance.stageSaveData.chapterAndStageStates[chapterIndex + 1].isCompleted)
        {
        ...
        else if (chapterIndex != StageManager.Instance.stageSaveData.currentChapterIndex) // 현재 챕터와 선택한 챕터가 다를 때만 초기화합니다.
        {
            ...
            StageManager.Instance.ResetStageData(chapterIndex); // 스테이지 데이터를 초기화합니다.
        }
